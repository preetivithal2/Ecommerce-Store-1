...
const displayCards = async (category) => {
  try {
    const response = await fetch("json/accessories.json");
    ...
    if (items) {
      items.forEach(item => {
        ...
        card.innerHTML = `
          ...
          <h3>${item.name}</h3>
          <p>${item.price}</p>
          <button class="buy-now-button bg-warning w-75 d-flex justify-content-center" style="border:none; position: relative;">Buy Now</button>
        `;
